...
import { GroupForm, GroupFormBody, GroupFormItemBody, UpdateGroupFormParams } from "../../../../types/groupForms";
import { showGroupFormRequest, updateGroupFormRequest } from "../../../../store/features/groupForms/groupFormsSlice";
...
const UpdateGroupFormPage: React.FC = () => {
    ...
    useEffect(() => {
        ...
        dispatch(showGroupFormRequest(formId ?? ""));
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, []);

    useEffect(() => {
        if (!form.showSuccess) return;
        setItems(form.data?.items ?? []);
        setMonth(form.data?.month ?? 0);
        setYear(form.data?.year ?? 0);
        setErrorsMap({});
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [form.showSuccess]);
    ...
    const makeRequest = () => {
        ...
        var params: UpdateGroupFormParams = {
            id: formId ?? "",
            body: body,
        }

        dispatch(updateGroupFormRequest(params));
    }
    ...
    if (grupo.loading || form.loading) return <div>Cargando...</div>;
...
export default UpdateGroupFormPage;
