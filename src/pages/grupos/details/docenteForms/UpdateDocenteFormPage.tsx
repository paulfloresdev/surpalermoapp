...
import { DocenteForm, DocenteFormBody, UpdateDocenteFormParams } from "../../../../types/docenteForms";
...
import { showDocenteFormRequest, updateDocenteFormRequest } from "../../../../store/features/docenteForms/docenteFormsSlice";
...
const UpdateDocenteFormPage: React.FC = () => {
    const { grupoId, formId } = useParams();
    ...
    useEffect(() => {
        ...
        dispatch(showDocenteFormRequest(formId ?? '0'));
    }, []);

    useEffect(() => {
        if (form.showSuccess !== null) {
            if (form.showSuccess) {
                setDateOf(form.data?.date_of ?? '');
                setA(form.data?.a ?? null);
                setB(form.data?.b ?? null);
                setC(form.data?.c ?? '');
                setD1(form.data?.d_1 ?? null);
                setD2(form.data?.d_2 ?? null);
                setD3(form.data?.d_3 ?? null);
                setD4(form.data?.d_4 ?? null);
                setD5(form.data?.d_5 ?? null);
                setD6(form.data?.d_6 ?? null);
                setD7(form.data?.d_7 ?? null);
                setD8(form.data?.d_8 ?? null);
                setD9(form.data?.d_9 ?? null);
                setD10(form.data?.d_10 ?? null);
                setD11(form.data?.d_11 ?? null);
                setD12(form.data?.d_12 ?? null);
                setD13(form.data?.d_13 ?? null);
                setD14(form.data?.d_14 ?? null);
                setD15(form.data?.d_15 ?? null);
                setD16(form.data?.d_16 ?? null);
                setE(form.data?.e ?? '');
                setDocenteId(form.data?.docente_id?.toString() ?? '');
                setCoordinadorId(form.data?.coordinador_id?.toString() ?? '');
            }
        }
    }, [form.showSuccess])
    ...
    const onSubmit = (x: React.FormEvent<HTMLFormElement>) => {
        ...
        var params: UpdateDocenteFormParams = {
            id: formId ?? '0',
            body: body,
        }

        dispatch(updateDocenteFormRequest(params));
    }
    ...
    return (
        <form onSubmit={onSubmit} className="w-full flex flex-col gap-8">
            <div className="flex flex-row items-center gap-4">
                ...
                <span className="bg-gray-100 text-gray-500 border-gray-200 border-1.5 text-sm p-2 rounded-xl">#{formId}</span>
            </div>
...
export default UpdateDocenteFormPage;