...
import { paginatedDocentesRequest } from "../../store/features/docentes/docentesSlice";
...
import StoreDocenteModal from "./modals/StoreDocenteModal";
import UpdateDocenteModal from "./modals/UpdateDocenteModal";
import DestroyDocenteModal from "./modals/DestroyDocenteModal";
...
const DocentesPage: React.FC = () => {
    ...
    const docentes = useSelector((state: RootState) => state.docentes as PaginatedState<Docente>);
    ...
    const [updateDocente, setUpdateDocente] = useState<undefined | Docente>(undefined);
    const [destroyDocente, setDestroyDocente] = useState<undefined | Docente>(undefined);

    useEffect(() => {
        if (!openStore && !openUpdate && !openDestroy) {
            console.log('docentes change detected');
            if (shouldRefresh) {
                console.log('docentes refreshed');
                makeSearch();
            }
        }
    ...
    useEffect(() => {
        if (updateDocente !== undefined && updateDocente !== null) {
            setOpenUpdate(true);
        }
    }, [updateDocente]);
    ...
    const makeSearch = () => {
        ...
        dispatch(paginatedDocentesRequest(params));
    }
    ...
    const downloadExcel = async () => {
        ...
        try {
            ...
            const filename = filenameFromHeader ?? "docentes.xlsx";
    ...
    return (
        <div className="w-full mx-auto h-full flex flex-col gap-2">
            <span className="font-semibold text-lg">Docentes</span>
                ...
                <div className="">
                    {
                        docentes.loading ?
                            ...
                            : docentes.error ? <span>{docentes.error}</span>
                                : <div className="flex flex-row items-center gap-2">
                                    <span className="text-sm text-gray-500">{`${docentes.data?.from} - ${docentes.data?.to} de ${docentes.data?.total}`}</span>
                                    <Pagination
                                        size="sm"
                                        variant="flat"
                                        classNames={{
                                        ...
                                        page={docentes.data?.current_page}
            ...
            {
                openUpdate && <UpdateDocenteModal
                    item={updateDocente!}
                    setItem={setUpdateDocente}
                    value={openUpdate}
            ...
            {
                openDestroy && <DestroyDocenteModal
                    item={destroyDocente!}
